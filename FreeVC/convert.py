...
from models import SynthesizerTrn, extract_speaker_embedding
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for line in tqdm(zip(titles, srcs, tgts)):
            ...
            if hps.model.use_spk:
                g_tgt = extract_speaker_embedding(tgt).to(
                    device
                )  # Ensure g_tgt is on the correct device
                g_tgt = g_tgt.unsqueeze(0).unsqueeze(-1)  # Match expected shape
            else:
                wav_tgt = torch.from_numpy(wav_tgt).to(device).unsqueeze(0)
                mel_tgt = mel_spectrogram_torch(
                    ...
                    device
                )  # Move mel spectrogram to device

            # src
            wav_src, _ = librosa.load(src, sr=hps.data.sampling_rate)
            wav_src = torch.from_numpy(wav_src).to(device).unsqueeze(0)
            c = utils.get_content(cmodel, wav_src).to(
                device
            )  # Move content representation to device

            c_lengths = (torch.ones(c.size(0)) * c.size(-1)).to(
                device
            )  # Move c_lengths to device
            ...
            if hps.model.use_spk:
                ...
                z_p, _, _, c_mask = net_g.enc_p(c, c_lengths)
                z = net_g.flow(z_p, c_mask, g=None, reverse=True)
                audio = net_g.dec(
                    z * c_mask, g=None
                )  # Ensure everything is on the same device

            audio = (
                audio[0][0].data.cpu().float().numpy()
