...
def load_checkpoint(checkpoint_path, model, optimizer=None, strict=False):
    """Loads a checkpoint while handling mismatched tensor sizes dynamically."""
    ...
    for k, v in state_dict.items():
        if k in saved_state_dict:
            ...
            # Handle shape mismatch dynamically
            if saved_v.shape != v.shape:
                logger.warning(
                    f"Shape mismatch in {k}: {saved_v.shape} -> {v.shape}. Adjusting..."
                )

                # Case: Expanding dimensions (e.g., 256 → 512)
                if saved_v.shape[1] < v.shape[1]:
                    new_v = torch.zeros_like(
                        v
                    )  # Create new tensor with the correct shape
                    new_v[:, : saved_v.shape[1], :] = saved_v  # Copy existing values
                else:
                    new_v = saved_v[
                        :, : v.shape[1], :
                    ]  # Trim excess dimensions if needed

                saved_v = new_v  # Replace with adjusted tensor
            new_state_dict[k] = saved_v
        else:
            logger.warning(
                f"{k} not found in checkpoint, using default initialization."
            )
            new_state_dict[k] = v  # Keep the default initialized value
